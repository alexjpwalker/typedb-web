...
files.forEach(file => {
  ...
  html = html.replace(/<style[^>]*>[\s\S]*?<\/style>/g, '');
    ...
    const foucShield = '<style id="fouc-shield">html{visibility:hidden;opacity:0}</style><script>const show=()=>{const el = document.getElementById("fouc-shield"); if (el) el.remove();};window.addEventListener("DOMContentLoaded",show);setTimeout(show,2000);</script>';
